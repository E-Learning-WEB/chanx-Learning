			...
			if(!empty($data->num_rows))
				{
					...
					include 'head.php';
					header("location:index.php?psn_login=1");
				}#3	
			
			//jika user tidak ada	
			else #3.1
				{
					header("location:index.php?sub=login&psn_login=2");
				}#3.1
...
if(isset($_POST['lupapassword1']))
	{
		include $engine->config('koneksi');
		$sql = sprintf("SELECT * FROM tb_pengguna WHERE username = '%s'", $_POST['username-lupapassword']);
		$data = $db->query($sql);
		...
		if(!empty($data->num_rows))
		{
			...
			$sql = sprintf("SELECT kode_lupapassword FROM tb_pengguna WHERE username = '%s'", $_POST['username-lupapassword']);
			$data = $db2->query($sql);
			$record = $data->fetch();
			$kodelupapassword_baru = $record['kode_lupapassword'];
			$kodelupapassword_baru_array = explode(":",$kodelupapassword_sekarang);
			$kode = $kodelupapassword_baru['0'];
			$waktu = $kodelupapassword_baru['1'];
		...
		{
			//header("location:login.php?psn_login=11"); //redirek (kode:11=username tidak ditemukan)
		}
	}
